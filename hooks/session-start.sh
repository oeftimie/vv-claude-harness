...
try:
    ...
    # real on this repo, 921 measured adversarially) -- cap it with the same
    # truncate-and-point treatment, marker naming the path count since the
    # full list is a list, not prose.
    SCOPE_LIMIT = 150
    if len(scope) > SCOPE_LIMIT:
        scope = scope[:SCOPE_LIMIT] + f"... ({len(scope_list)} paths total, see .harness/features.json)"
